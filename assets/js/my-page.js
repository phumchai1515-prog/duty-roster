...
import {
  ...
  respondToSwap, cancelSwapRequest, loadColleagues,
} from './swaps.js';
import { formatThaiDateShort, formatThaiDateFull, todayKey, parseDateKey, DOW_TH, dayOfWeek } from './thai.js';
import { SLOTS, STATUS_LABEL, STATUS_PILL } from './config.js';
...
function renderUpcoming(groups, outgoing) {
  ...
  dom.summary.textContent = `ทั้งหมด ${groups.length} เวร · รออนุมัติ ${pending} เวร`;

  dom.upcoming.innerHTML = groups.map((group) => {
    const swap = group.ids.map((id) => outgoing.get(id)).find(Boolean);
    const canCancel = group.status === 'pending';
    ...
    return `
      <tr>
        ...
        <td><div class="row" style="flex-wrap:nowrap">${actions}</div></td>
      </tr>
    `;
  }).join('');
}
...
async function refresh() {
  ...
  try {
    const [shifts, incoming, outgoing] = await Promise.all([
...
function openSwapDialog(shiftId, dutyDate) {
...
async function sendSwap() {
  ...
  try {
    ...
    toast('ส่งคำขอแลกเวรแล้ว', 'success');
    await refresh();
  } catch (error) {
...
dom.upcoming.addEventListener('click', async (event) => {
  ...
  if (action === 'swap') {
    openSwapDialog(button.dataset.id, button.dataset.date);
    return;
  }
  ...
  try {
    if (action === 'cancel') {
      if (!window.confirm('ยกเลิกการจองเวรวันนี้ใช่หรือไม่?')) {
...
dom.incoming.addEventListener('click', async (event) => {
  ...
  if (accept && !window.confirm('รับเวรนี้แทนใช่หรือไม่? เวรจะถูกโอนมาเป็นของคุณ')) return;

  button.disabled = true;
  try {
    await respondToSwap(button.dataset.id, accept);
    toast(accept ? 'รับเวรเรียบร้อย รอหัวหน้าอนุมัติ' : 'ปฏิเสธคำขอแล้ว', 'success');
    await refresh();
  } catch (error) {
